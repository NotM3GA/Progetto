        ...
        switch (modalita) {
        	...
        	case GIOCO:
		        ...
		        {		        	
		            ...
		            if (fruit.active) DrawTexture(fruit.im1, fruit.position.x, fruit.position.y, fruit.color); 
		            
		             DrawText(TextFormat("SCORE: %04i", score), 10, 700, 25, darkGreen);
		              DrawText(TextFormat("Retro Game"), 400, 700, 25, darkGreen);
		             
		            if (pause) DrawText("GAME PAUSE", screenWidth/2 - MeasureText("GAME PAUSE", 40)/2, screenHeight/2 - 40, 40, GRAY);
